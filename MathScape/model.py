...
# openai设置
class openai_config:
    openai_api = os.environ.get('OPENAI_API_ADDR', 'http://123.129.219.111:3000/v1')
...
class Qwen2VLModel(Model):
    def __init__(self, server, name='qwen2-vl-instruct', timeout=180, temperature=0.3, max_tokens=2048):
        super().__init__(server, name, stream=False, timeout=timeout, temperature=temperature)
        self.api_key = api_manager.get_keys('qwen')
    ...
    def set_api_key(self, api_key):
        self.api_key = api_key
        api_manager.add_key('qwen', api_key)
    ...
    def call(self, prompt, history=[], system=None, qtype=''):
        ...
        headers = {'Content-Type': 'application/json'}
        if self.api_key:
            api_key = self.api_key if isinstance(self.api_key, str) else random.choice(self.api_key)
            headers['Authorization'] = f'Bearer {api_key}'
    ...
    def call_details(self, prompt, history=[], system=None, qtype=''):
        result_text = self.call(prompt, history, system, qtype)
        return {
            'status': 'finish' if result_text else 'failed',
            'text': result_text,
            'cost': 0
        }
...
def get_base64_image(path):
    if os.path.exists(path):
        try:
            ...
            return img_str
        except Exception as e:  # 捕获所有图片处理错误
            print(f"Error loading image {path}: {e}")
            return None  # 返回 None，代码会继续只用文本处理
    else:
        return None
...
def process_ID(data):
    ...
    model = get_model('qwen2.5-vl-7b')
    for key,value in data.items():
        id=key
        imageid = value.get("image_id") 
        image_name = str(imageid) + '.png'
...
def parallel_call_cache(data,input_file,out_file, max_workers=10):
    ...
    with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in futures.as_completed(process_futures):
            ...
            with open("parallel_qwen2.5vl-7b_ans.jsonl", 'a', encoding='utf-8') as f:
...
def get_model(model_name, temperature=None, timeout=None):
    ...
    elif model_config['type'] == 'qwen2-vl':
        return Qwen2VLModel(
            server=model_config['server'],
            name=model_config.get('name', 'qwen2-vl-instruct'),
            timeout=timeout if timeout else model_config.get('timeout', 180),
            temperature=temperature if temperature else model_config.get('temperature', 0.3),
            max_tokens=model_config.get('max_tokens', 2048)
        )

input_file='./Math_data/math_question_solution_ans.json'
out_file=''
